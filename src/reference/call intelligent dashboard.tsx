...
import { Phone, Clock, Calendar, TrendingUp, AlertCircle, CheckCircle, XCircle, PhoneCall, PhoneMissed, BarChart3, Target, MessageSquare, Filter, ChevronRight, Play, User, DollarSign, Home, Users, Heart, Briefcase, ChevronDown, ChevronUp, AlertTriangle, Sparkles, Shield, Building } from 'lucide-react';

const BrokerCallAnalysisDashboard = () => {
  const [timeFilter, setTimeFilter] = useState('today');
  const [selectedCall, setSelectedCall] = useState(null);
  const [expandedCall, setExpandedCall] = useState(null);

  // Enhanced call data structure matching the AI extraction
  const callData = [
    {
      id: 1,
      ...
      // Basic Analysis
      callSummary: "Client interested in renewal options with current rate at 4.79%. Concerned about rising rates and wants to lock in soon. Also mentioned planning kitchen renovations this summer.",
      callType: "renewal",
      loanAmount: "$385,000",
      ...
      objections: [
        {
          ...
          broker_response: "I can likely beat that rate and explained our lender has better prepayment options",
          objection_timestamp: "[05:23]",
          response_timestamp: "[05:45]"
        },
        {
          objection: "I need to talk to my husband first",
          broker_response: "Suggested scheduling a call with both of them tomorrow evening",
      ...
      // Client Profile
      clientProfile: {
        employmentMentioned: "Both working, husband just promoted to senior manager",
      ...
      strengths: [
        {
          strength: "Built great rapport by remembering daughter's university plans",
          timestamp: "[02:15]"
        },
        {
    ...
    },
    {
      ...
      callSummary: "Left voicemail for new purchase lead. Web inquiry from yesterday looking for pre-approval on $650k property.",
      callType: "new_lead",
      loanAmount: "$520,000",
      outcome: "voicemail",
      urgencyLevel: "medium",
    ...
    },
    {
      ...
      callSummary: "Existing client looking to refinance and pull equity for investment property. Strong financial position but concerned about qualifying with rental property.",
      callType: "refinance",
      loanAmount: "$425,000",
      outcome: "quoted",
      urgencyLevel: "medium",
      ...
      objections: [
        {
          ...
          broker_response: "Showed how rental income would offset this in 18 months",
      ...
      crossSellOpportunities: [
        {
          ...
          reason: "Retirement planning mentioned - building investment portfolio"
        },
        {
  ...
  const stats = {
    ...
    connected: callData.filter(c => c.outcome !== 'voicemail' && c.outcome !== 'no_answer').length,
    applications: callData.filter(c => c.outcome === 'application_started').length,
    avgObjHandling: 85, // % of objections handled well
    crossSellsIdentified: callData.reduce((acc, call) => acc + call.crossSellOpportunities.length, 0),
  ...
  const getOutcomeColor = (outcome) => {
    switch(outcome) {
      case 'application_started': return 'text-green-700 bg-green-100';
      case 'quoted': return 'text-blue-700 bg-blue-100';
      case 'follow_up_scheduled': return 'text-purple-700 bg-purple-100';
      case 'not_interested': return 'text-red-700 bg-red-100';
      case 'voicemail': return 'text-yellow-700 bg-yellow-100';
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      call.outcome === 'voicemail' ? 'bg-yellow-100' :
                      call.outcome === 'application_started' ? 'bg-green-100' :
                      'bg-blue-100'
                    }`}>
                    ...
                    <div>
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getUrgencyColor(call.urgencyLevel)}`}>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-gray-900">{call.nextSteps}</span>
                    {call.followUpDate && (
                      <span className="text-sm text-gray-500">• Due: {call.followUpDate}</span>
                    )}
                  </div>
                  {call.duration !== "0:45" && (
                    ...
                    <div className="space-y-4">
                      ...
                      {call.propertyDetails.propertyType && (
                        <div>
                          ...
                          <div className="bg-white rounded-lg p-3 space-y-1 text-sm">
                            <div><span className="text-gray-600">Type:</span> <span className="font-medium">{call.propertyDetails.propertyType}</span></div>
                            <div><span className="text-gray-600">Value:</span> <span className="font-medium">{call.propertyDetails.propertyValue}</span></div>
                            <div><span className="text-gray-600">Location:</span> <span className="font-medium">{call.propertyDetails.location}</span></div>
                          </div>
                        </div>
                      )}
                      ...
                      {call.clientProfile.employmentMentioned && (
                        <div>
                          ...
                          <div className="bg-white rounded-lg p-3 space-y-1 text-sm">
                            <div><span className="text-gray-600">Employment:</span> <span className="font-medium">{call.clientProfile.employmentMentioned}</span></div>
                            <div><span className="text-gray-600">Family:</span> <span className="font-medium">{call.clientProfile.familyStatus}</span></div>
                            <div><span className="text-gray-600">Goals:</span> <span className="font-medium">{call.clientProfile.financialGoals}</span></div>
                          </div>
                        </div>
                      )}
                          ...
                          <div className="space-y-2">
                            {call.objections.map((obj, idx) => (
                              <div key={idx} className="bg-white rounded-lg p-3 text-sm">
                                <div className="text-red-700 font-medium">{obj.objection}</div>
                                <div className="text-green-700 mt-1">→ {obj.broker_response}</div>
                          ...
                          <div className="bg-green-50 rounded-lg p-3 space-y-1">
                            {call.strengths.map((strength, idx) => (
                              <div key={idx} className="text-sm flex items-start gap-2">
                                ...
                                <span>{strength.strength} {strength.timestamp}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            {call.crossSellOpportunities.map((opp, idx) => (
                              <div key={idx} className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                                ...
                                <div className="text-sm text-purple-700 mt-1">{opp.reason}</div>
                      ...
                      {call.rateDiscussion.currentRate && (
                        <div>
                          ...
                          <div className="bg-white rounded-lg p-3 space-y-1 text-sm">
                            <div><span className="text-gray-600">Current:</span> <span className="font-medium">{call.rateDiscussion.currentRate}</span></div>
                            <div><span className="text-gray-600">Desired:</span> <span className="font-medium">{call.rateDiscussion.desiredRate}</span></div>
                            <div><span className="text-gray-600">Rate Sensitive:</span> <span className={`font-medium ${call.rateDiscussion.rateSensitive ? 'text-red-600' : 'text-green-600'}`}>{call.rateDiscussion.rateSensitive ? 'Yes' : 'No'}</span></div>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="bg-white rounded-lg p-3 space-y-1 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className={`font-medium ${
                              call.callQuality.clientEngagement === 'high' ? 'text-green-600' :
                              call.callQuality.clientEngagement === 'medium' ? 'text-yellow-600' :
                              'text-red-600'
                            }`}>{call.callQuality.clientEngagement}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Next Contact:</span>
                            <span className="font-medium">{call.callQuality.nextContactAgreed ? 'Yes' : 'No'}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Timeline Given:</span>
                            <span className="font-medium">{call.callQuality.specificTimeframeGiven ? 'Yes' : 'No'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
...
export default BrokerCallAnalysisDashboard;